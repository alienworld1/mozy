...
export async function inspectProtocol(values: Record<string, string>, output: ProtocolOutput): Promise<void> {
  ...
  const settlement = new Contract(deployment.contracts.settlement, settlementArtifact.abi, provider);

  output.phase("Checking live protocol wiring");
  const [registryCode, marketCode, vaultCode, settlementCode, registryOwner, marketOwner, wiredVault, wiredSettlement, vaultOperator, settlementMarket, settlementVerifier, chainInfo, sourceWindowBlocks, graceBlocks, configHash, marketConfig, nextMandateId, nextReservationId, bondRateBps, bondCap] = await Promise.all([
    provider.getCode(deployment.contracts.registry), provider.getCode(deployment.contracts.market), provider.getCode(deployment.contracts.vault), provider.getCode(deployment.contracts.settlement),
    registry.owner() as Promise<string>, market.owner() as Promise<string>, market.vault() as Promise<string>, market.settlementOperator() as Promise<string>, vault.marketOperator() as Promise<string>,
    settlement.market() as Promise<string>, settlement.verifier() as Promise<string>, market.chainInfo() as Promise<string>, market.sourceWindowBlocks() as Promise<bigint>, market.settlementGraceBlocks() as Promise<bigint>,
  ...
  if ([registryCode, marketCode, vaultCode, settlementCode].some((code) => code === "0x")) throw mismatch("One or more recorded protocol addresses have no code.");
  ...
  if (
    getAddress(wiredSettlement) !== getAddress(deployment.contracts.settlement)
    || getAddress(settlementMarket) !== getAddress(deployment.contracts.market)
    || getAddress(settlementVerifier) !== getAddress(deployment.contracts.verifier)
    || getAddress(chainInfo) !== getAddress(deployment.contracts.chainInfo)
  ) throw mismatch("The settlement and authenticated-height wiring does not reconcile.");
  ...
  if (
    deployment.deployedAt !== "address-input"
    && (sourceWindowBlocks.toString() !== deployment.sourceWindowPolicy.acceptedBlocks
      || graceBlocks.toString() !== deployment.sourceWindowPolicy.settlementGraceBlocks)
  ) throw mismatch("Live source-height policy differs from the deployment artifact.");
  ...
  output.phase("Accepted source window", `${sourceWindowBlocks} blocks, inclusive`);
  ...
  if (receiptInput) {
    if (!/^0x[0-9a-fA-F]{64}$/.test(receiptInput)) throw new ProtocolCommandError("Validating receipt identity", "Receipt identity must be exactly 32 bytes.", "Pass the canonical replay identity from settlement output.");
    const boundReservation = await settlement.receiptReservation(receiptInput) as bigint;
    output.phase("Receipt consumption", boundReservation === 0n ? "Receipt has not been consumed" : `Consumed by reservation ${boundReservation}`);
  }
}
...
async function inspectReservation(idValue: bigint, market: Contract, vault: Contract, output: ProtocolOutput): Promise<void> {
  ...
  output.phase("Foreign token decimals", (marketConfig[2] as bigint).toString());
  output.phase("Settlement token", token);
  output.phase("Settlement token decimals", (marketConfig[4] as bigint).toString());
  ...
  output.phase("Expiry eligible height", (reservation[10] as bigint).toString());
...
function contractErrorName(error: unknown, contract: Contract): string | undefined {
...
async function resolveDeployment(values: Record<string, string>): Promise<ProtocolDeploymentArtifact> {
  const suppliedAddresses = [values.registry, values.market, values.vault, values.settlement, values.admin].filter(Boolean).length;
  if (suppliedAddresses > 0 && suppliedAddresses < 5) {
    throw new ProtocolCommandError("Loading deployment addresses", "Direct inspection requires registry, market, vault, settlement, and admin together.", "Pass all five public values or omit them to use the canonical artifact.");
  }
  if (values.registry && values.market && values.vault && values.settlement && values.admin) {
    return {
      schemaVersion: "3", environmentConfigVersion: config.configVersion, environmentConfigHash: id(config.configVersion), chainId: config.creditcoin.chainId,
      ...
      contracts: { registry: getAddress(values.registry), market: getAddress(values.market), vault: getAddress(values.vault), settlement: getAddress(values.settlement), verifier: config.attestcoin.verifierAddress, chainInfo: config.attestcoin.chainInfoAddress, decoder: config.attestcoin.decoderAddress }, marketId: "1",
      bondPolicy: { rateBps: "", cap: "", denominator: "10000" },
      sourceWindowPolicy: { acceptedBlocks: "", settlementGraceBlocks: "", bounds: "inclusive" },
      transactions: { registryDeployment: "", marketDeployment: "", vaultDeployment: "", settlementDeployment: "", vaultConfiguration: "", settlementConfiguration: "", marketConfiguration: "" },
    };
  }
  try {
    return JSON.parse(await readFile(path.join(projectRoot, "artifacts", "protocol", "cc3-settlement.json"), "utf8")) as ProtocolDeploymentArtifact;
  } catch {
